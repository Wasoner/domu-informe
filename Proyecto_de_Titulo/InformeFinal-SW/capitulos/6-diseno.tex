...
Las 34~tablas del modelo, derivadas de las 35~migraciones Flyway del proyecto, se organizan en 11~dominios funcionales vigentes. La Tabla~\ref{tab:resumen-dominios} presenta un resumen de esta distribución; el detalle de cada entidad y sus atributos se encuentra en el Anexo~\ref{anexo:diccionario} (Diccionario de Datos).
...
\begin{tabular}{|p{0.32\textwidth}|p{0.36\textwidth}|p{0.24\textwidth}|}
\hline
...
Ya que el proyecto utiliza MySQL como motor de base de datos, se presenta a continuación el diagrama Entidad-Relación simplificado del sistema (Figura~\ref{fig:mer}). El diagrama agrupa las entidades por dominio funcional (colores) y muestra las relaciones con notación \textit{crow's foot}; los atributos de cada entidad se detallan en el Anexo~\ref{anexo:diccionario} (Diccionario de Datos).

\begin{landscape}
\input{figures/mer-tikz}
\end{landscape}
...
A continuación se presenta el Modelo Relacional completo del sistema DOMU (Figura~\ref{fig:mr-detalle}), mostrando las 34 tablas con sus atributos, claves primarias y foráneas, y las relaciones de cardinalidad entre ellas. El detalle de cada columna se encuentra en el Anexo~\ref{anexo:diccionario} (Diccionario de Datos).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{MER-nuevo.png}
  \caption{Modelo Relacional completo del sistema DOMU.}
  \label{fig:mr-detalle}
\end{figure}
...
Las imagenes que siguen a continuación corresponden a \textbf{capturas de pantallas del sistema}, obtenidas desde el \textbf{navegador web} (vista de escritorio).
 El orden presenta un recorrido coherente desde el acceso al sistema hasta módulos de comunidad, administración y operación diaria.

\newpage
% --- A: Inicio de Sesión ---
\begin{itemize}
    \item[\textbf{A.}] \textbf{Inicio de sesión}: Pantalla de autenticación para ingreso de credenciales de usuarios autorizados.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{Iniciar sesion.png}
    \caption{A. Vista en escritorio: inicio de sesión.}
...
% --- B: Home principal ---
\begin{itemize}
    \item[\textbf{B.}] \textbf{Página principal (home)}: Punto de entrada tras el inicio de sesión, con accesos a las funcionalidades principales de la plataforma.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{HomePrincipalDomu.png}
    \caption{B. Vista en escritorio: página principal DOMU.}
    \label{fig:captura-home}
\end{figure}
\clearpage

% --- C: Panel administración ---
\begin{itemize}
    \item[\textbf{C.}] \textbf{Panel principal de administración}: Tablero orientado a roles administrativos para supervisar y operar la comunidad.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{PanelPrincipalAdmin.png}
    \caption{C. Vista en escritorio: panel principal de administración.}
    \label{fig:captura-panel-admin}
\end{figure}
\clearpage

% --- D: Registro de visitas ---
\begin{itemize}
    \item[\textbf{D.}] \textbf{Registro de visitas}: Módulo operativo para el control de ingreso de visitas a la comunidad.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{RegistroVisitas.png}
    \caption{D. Vista en escritorio: registro de visitas.}
    \label{fig:captura-registro-visitas}
\end{figure}
\clearpage

% --- E: Gestión de personal ---
\begin{itemize}
    \item[\textbf{E.}] \textbf{Gestión de personal}: Administración de fichas y roles del personal asociado a la comunidad.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{GestionPersonal.png}
    \caption{E. Vista en escritorio: gestión de personal.}
    \label{fig:captura-gestion-personal}
\end{figure}
\clearpage

% --- F: Gestión de unidades ---
\begin{itemize}
    \item[\textbf{F.}] \textbf{Gestión de unidades}: Mantención de departamentos o unidades habitacionales dentro del conjunto residencial.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{GestionUnidades.png}
    \caption{F. Vista en escritorio: gestión de unidades.}
    \label{fig:captura-gestion-unidades}
\end{figure}
\clearpage

% --- G: Crear usuarios ---
\begin{itemize}
    \item[\textbf{G.}] \textbf{Creación de usuarios}: Alta de cuentas y asignación de perfiles para residentes, administración u otros roles habilitados.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{CrearUsuarios.png}
    \caption{G. Vista en escritorio: creación de usuarios.}
    \label{fig:captura-crear-usuarios}
\end{figure}
\clearpage

% --- H: Asignar tareas ---
\begin{itemize}
    \item[\textbf{H.}] \textbf{Asignación de tareas}: Coordinación de labores o encargos entre actores de la comunidad (por ejemplo, conserjería u operaciones).
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{AsignarTareas.png}
    \caption{H. Vista en escritorio: asignación de tareas.}
    \label{fig:captura-asignar-tareas}
\end{figure}
\clearpage

% --- I: Muro comunidad ---
\begin{itemize}
    \item[\textbf{I.}] \textbf{Muro de la comunidad}: Espacio de comunicación y publicaciones entre vecinos o entre administración y residentes.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{MuroComunidad.png}
    \caption{I. Vista en escritorio: muro de la comunidad.}
    \label{fig:captura-muro}
\end{figure}
\clearpage

% --- J: Marketplace ---
\begin{itemize}
    \item[\textbf{J.}] \textbf{Marketplace}: Sección para ofertas, trueques o avisos comerciales entre miembros de la comunidad, según la configuración del sistema.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{Marketplace.png}
    \caption{J. Vista en escritorio: marketplace.}
    \label{fig:captura-marketplace}
\end{figure}
\clearpage

% --- K: Perfil ---
\begin{itemize}
    \item[\textbf{K.}] \textbf{Perfil de usuario}: Consulta y edición de datos personales y preferencias asociadas a la cuenta.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{Perfil.png}
    \caption{K. Vista en escritorio: perfil de usuario.}
    \label{fig:captura-perfil}
\end{figure}
\clearpage

% --- L: Reportar incidencias ---
\begin{itemize}
    \item[\textbf{L.}] \textbf{Reporte de incidencias}: Registro y seguimiento de reclamos, sugerencias o fallas operacionales (\textit{tickets}).
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,keepaspectratio]{ReportarIncidentes.png}
    \caption{L. Vista en escritorio: reporte de incidencias.}
    \label{fig:captura-incidencias}
\end{figure}
\clearpage
